...
clf = LogisticRegression(solver="saga",multi_class="mutinomial").fit(X_train_scaled,y_train)

def get_prediction(image):
    im_pil = Image.open(image)
    image_bw = im_pil.convert('L')
    image_bw_resized = image_bw_resize((22,30),Image.ANTIALIAS)
    pixel_filter = 20
